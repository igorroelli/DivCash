...
const ExpenseItem = ({ item, onDelete, onShowReceipt }) => {
  const currentUser = auth.currentUser;
  const debtors = item.participants.filter(p => p.uid !== item.payerId);
    ...
    <View style={styles.expenseItem}>
      <View style={styles.expenseHeader}>
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          {item.receiptUrl && (
            <TouchableOpacity onPress={() => onShowReceipt(item.receiptUrl)} style={{ marginRight: 10 }}>
      ...
      <Text style={styles.paidBy}>Pago por: {item.payerNickname || 'Desconhecido'}</Text>
      
      {item.type === 'carro' && (
        <View style={styles.carDetailsContainer}>
          <Text style={styles.carDetailText}>Distância: {item.distance || 0} km</Text>
          {item.tollCost > 0 && (
             <Text style={styles.carDetailText}>Pedágios: {item.tollCount || 0} (R$ {item.tollCost?.toFixed(2)})</Text>
          )}
        </View>
      )}
      ...
      {debtors.length > 0 && (
        <View style={styles.debtorsContainer}>
          ...
          {debtors.map(debtor => (
            <Text key={debtor?.uid} style={styles.debtorName}>- {debtor.nickname || 'Usuário'} (R$ {debtor.amountOwed?.toFixed(2) || '0.00'})</Text>
          ))}
        </View>
      )}
    </View>
...
export default function GroupBalanceScreen({ route }) {
  ...
  const handleShowReceipt = (uri) => {
    setModalImageUri(uri);
    setIsModalVisible(true);
  };
  
  const handleDeleteExpense = (expenseId, description) => {
    Alert.alert( "Excluir Despesa", `Você tem certeza que deseja excluir a despesa "${description}"?`,
      [ { text: "Cancelar", style: "cancel" }, { text: "Excluir", style: "destructive",
          onPress: async () => {
            const expenseRef = doc(db, 'groups', groupId, 'expenses', expenseId);
            try { await deleteDoc(expenseRef); }
            catch (error) { console.error("Erro ao excluir despesa: ", error); Alert.alert("Erro", "Não foi possível excluir a despesa."); }
          }
        }
      ]
    );
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      try {
        ...
        console.error("Erro crítico no cálculo do balanço:", error);
        setLoading(false);
      }
    }, (error) => {
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <ScrollView style={styles.container}>
        ...
        {expenses.length > 0 ? (
          expenses.map(item => <ExpenseItem key={item?.id} item={item} onDelete={handleDeleteExpense} onShowReceipt={handleShowReceipt} />)
        ) : (
...
const styles = StyleSheet.create({
  ...
  carDetailsContainer: { marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: '#f5f5f5' },
  carDetailText: { fontSize: 12, color: '#666', fontStyle: 'italic' },
