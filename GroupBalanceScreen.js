...
export default function GroupBalanceScreen({ route }) {
      ...
      for (const uid of memberIds) {
        ...
        for (const otherUid of memberIds) {
          if (uid !== otherUid) {
            ...
            if (debtAmount > 0.01) {
              memberData.debts.push({ to: memberProfiles[otherUid]?.nickname, amount: debtAmount });
            }
            const creditAmount = matrix[otherUid]?.[uid] || 0;
            if (creditAmount > 0.01) {
              memberData.credits.push({ from: memberProfiles[otherUid]?.nickname, amount: creditAmount });
            }
          }
        }
  ...
  return (
    <ScrollView style={styles.container}>
      {/* Seção de Balanço Detalhado por Pessoa */}
