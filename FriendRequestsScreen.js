  ...
  const handleResponse = async (request, newStatus) => {
    ...
    if (newStatus === 'accepted') {
      try {
        ...
        if (!currentUserDoc.exists()) throw new Error("Perfil do usuário atual não encontrado.");
        const myProfile = currentUserDoc.data();

        const myFriendsRef = doc(db, 'users', currentUser.uid, 'friends', request.from);
        await setDoc(myFriendsRef, { uid: request.from, email: request.fromEmail, nickname: request.fromNickname });

        const theirFriendsRef = doc(db, 'users', request.from, 'friends', currentUser.uid);
        await setDoc(theirFriendsRef, { uid: myProfile.uid, email: myProfile.email, nickname: myProfile.nickname });
      } catch (error) {
        console.error("Erro ao criar amizade mútua:", error);
        Alert.alert("Erro", "Ocorreu um problema ao adicionar o amigo.");
      }
    }
  };
        ...
        renderItem={({ item }) => (
          <View style={styles.requestItem}>
            ...
            <View style={styles.buttonsContainer}>
              <TouchableOpacity style={[styles.responseButton, styles.acceptButton]} onPress={() => handleResponse(item, 'accepted')}><Text style={styles.buttonText}>Aceitar</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.responseButton, styles.declineButton]} onPress={() => handleResponse(item, 'declined')}><Text style={styles.buttonText}>Recusar</Text></TouchableOpacity>
            </View>
          </View>
        )}
