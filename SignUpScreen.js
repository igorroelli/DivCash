  ...
  const handleSignUp = () => {
    ...
    createUserWithEmailAndPassword(auth, email, senha)
      .then((userCredential) => {
        ...
        // Cria o documento do usuário SEM o apelido
        setDoc(doc(db, "users", user.uid), {
          uid: user.uid,
          email: user.email.toLowerCase(),
        });

        // Navega para a tela de carregamento que fará a verificação
      ...
      .catch((error) => {
        ...
        else if (error.code === 'auth/invalid-email') { friendlyMessage = 'O endereço de e-mail é inválido.'; }
