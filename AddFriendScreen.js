...
export default function AddFriendScreen() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      Alert.alert('Erro', 'Ocorreu um problema ao buscar. Verifique o índice do Firestore.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendFriendRequest = async (recipient) => {
    ...
    try {
      await addDoc(collection(db, 'friendRequests'), {
        from: currentUser.uid, fromEmail: currentUser.email, fromNickname: senderNickname,
        to: recipient.uid, toEmail: recipient.email, toNickname: recipient.nickname,
  ...
  return (
    <View style={styles.container}>
      <TextInput style={styles.input} placeholder="Buscar amigo por apelido" value={searchNickname} onChangeText={setSearchNickname} autoCapitalize="none" />
      ...
      {loading ? ( <ActivityIndicator size="large" color="#4CAF50" /> ) : (
        <FlatList
          ...
          ListEmptyComponent={ hasSearched ? <Text style={styles.emptyText}>Nenhum usuário encontrado.</Text> : null }
        />
      )}
    </View>
  );
