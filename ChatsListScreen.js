...
// Componente para renderizar cada item da lista de grupos
const GroupItem = ({ item, navigation, onDelete }) => (
  <TouchableOpacity 
    ...
    onLongPress={() => {
      Alert.alert(
        'Excluir Grupo',
        `Você tem certeza que deseja excluir o grupo "${item.name}"?`,
        [
...
// Componente principal que mostra a lista de conversas
export default function ChatsListScreen() {
  const navigation = useNavigation(); // Obtém a navegação, já que esta tela está dentro de um TabNavigator
  const { groups, deleteGroup, loading } = useGroups();
